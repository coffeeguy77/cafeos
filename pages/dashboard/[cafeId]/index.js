...
function EquipmentTab({cafeId,equipment,onRefresh}){
  ...
  return(<div>
    ...
    {showImport&&<div style={{background:'white',border:'1px solid var(--border)',borderRadius:'12px',padding:'1rem',marginBottom:'1rem'}}>
      ...
      <div style={{display:'flex',gap:'6px'}}><button onClick={doImport} disabled={importing||!csvPreview?.length} style={{fontSize:'12px',padding:'5px 14px',borderRadius:'7px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer',opacity:importing||!csvPreview?.length?0.6:1}}>{importing?'ImportingÃ¢ÂÂ¦':'Import '+(csvPreview?.length||0)+' items'}</button><button onClick={()=>{setShowImport(false);setCsvPreview(null);setImportErr('')}} style={{fontSize:'12px',padding:'5px 10px',borderRadius:'7px',background:'transparent',color:'var(--text-secondary)',border:'1px solid var(--border)',cursor:'pointer'}}>Cancel</button></div>
    </div>}
...
function SupplierMappingTab({cafeId,onSaved}){
  ...
  if(loading) return <div style={{textAlign:'center',padding:'3rem',color:'var(--text-muted)',fontSize:'13px'}}>Loading supplier invoices from XeroÃ¢ÂÂ¦</div>
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1rem'}}>
      <div>
        <h2 style={{fontSize:'19px',marginBottom:'3px'}}>Supplier COGS mapping</h2>
        <p style={{fontSize:'12px',color:'var(--text-muted)'}}>All suppliers with invoices in Xero, sorted by spend. Tick the ones that supply this cafÃÂ©.</p>
      </div>
      <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
        ...
        <button onClick={save} disabled={saving} style={{fontSize:'12px',padding:'5px 14px',borderRadius:'7px',background:saved?'var(--success)':'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer'}}>{saving?'SavingÃ¢ÂÂ¦':saved?'Ã¢ÂÂ Saved':'Save COGS suppliers'}</button>
      </div>
    </div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'8px',marginBottom:'1rem'}}>
      <div style={{background:'var(--success-light)',borderRadius:'9px',padding:'0.65rem 0.875rem',gridColumn:'span 2'}}>
        <p style={{fontSize:'10px',color:'var(--success)',fontWeight:600,marginBottom:'2px',textTransform:'uppercase'}}>CafÃÂ© COGS</p>
        <p style={{fontSize:'22px',fontFamily:'serif',color:'var(--success)'}}>{fmt(cafeTotal)}</p>
        <p style={{fontSize:'10px',color:'var(--success)',marginTop:'1px'}}>{selected.length} supplier{selected.length!==1?'s':''} ÃÂ· annualised {fmt(cafeTotal/months*12)}</p>
      </div>
      <div style={{background:'var(--crema-pale)',borderRadius:'9px',padding:'0.65rem 0.875rem'}}>
        ...
        <p style={{fontSize:'10px',color:'var(--text-muted)',marginTop:'1px'}}>All entities ÃÂ· {months}mo</p>
      </div>
      <div style={{background:'var(--crema-pale)',borderRadius:'9px',padding:'0.65rem 0.875rem'}}>
    ...
    <div style={{display:'flex',gap:'8px',marginBottom:'10px',alignItems:'center'}}>
      <input style={{...II,flex:1}} placeholder="Search suppliersÃ¢ÂÂ¦" value={search} onChange={e=>setSearch(e.target.value)}/>
    ...
    {suppliers.length===0&&<div style={{textAlign:'center',padding:'2.5rem',background:'white',borderRadius:'12px',border:'1px solid var(--border)'}}><div style={{fontSize:'32px',marginBottom:'8px'}}>Ã°ÂÂÂ¦</div><p style={{color:'var(--text-secondary)',fontSize:'13px',marginBottom:'4px'}}>No supplier invoices found in Xero for this period.</p><p style={{color:'var(--text-muted)',fontSize:'12px'}}>Make sure Xero is reconnected with the new credentials so supplier data can be accessed.</p></div>}

    {/* Selected first, then unselected */}
    <div style={{display:'grid',gap:'4px'}}>
      {filtered.filter(s=>mapping[s.name]).length>0&&<>
        <div style={{fontSize:'10px',color:'var(--success)',fontWeight:600,padding:'4px 2px',textTransform:'uppercase',letterSpacing:'.05em'}}>Selected Ã¢ÂÂ cafÃÂ© COGS suppliers</div>
        {filtered.filter(s=>mapping[s.name]).map(s=>{
          ...
          return(<div key={s.name} onClick={()=>toggle(s.name)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'9px 12px',borderRadius:'8px',border:'1px solid var(--success)',background:'#f0faf4',cursor:'pointer',marginBottom:'2px'}}>
            <div style={{width:'18px',height:'18px',borderRadius:'4px',border:'2px solid var(--success)',background:'var(--success)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{color:'white',fontSize:'11px',fontWeight:700}}>Ã¢ÂÂ</span></div>
      ...
      {filtered.filter(s=>!mapping[s.name]).map(s=>{
        ...
        return(<div key={s.name} onClick={()=>toggle(s.name)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'9px 12px',borderRadius:'8px',border:'1px solid var(--border)',background:'white',cursor:'pointer',marginBottom:'2px'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
            ...
            <span style={{fontSize:'13px',fontFamily:'serif',color:'var(--text-muted)',width:'90px',textAlign:'right'}}>{s.total>0?fmt(s.total):'Ã¢ÂÂ'}</span>
    ...
    {showForm&&<div style={{background:'white',border:'1px solid var(--crema)',borderRadius:'12px',padding:'1.25rem',marginBottom:'1rem'}}>
      ...
      <form onSubmit={save}>
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:'8px',marginBottom:'10px'}}>
          <div><label style={LB}>Label *</label><input style={II} value={form.label} onChange={e=>sf('label',e.target.value)} placeholder="e.g. CafÃÂ© share of rent" required/></div>
...
function XeroMappingTab({cafeId,onMappingSaved}){
  ...
  if(loading) return <div style={{textAlign:'center',padding:'3rem',color:'var(--text-muted)',fontSize:'13px'}}>Loading Xero P&LÃ¢ÂÂ¦</div>
  if(!raw) return <div style={{textAlign:'center',padding:'3rem',color:'var(--danger)',fontSize:'13px'}}>Could not load Xero data.</div>
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1rem'}}>
      <div><h2 style={{fontSize:'19px',marginBottom:'3px'}}>Xero P&L mapping</h2><p style={{fontSize:'12px',color:'var(--text-muted)'}}>Tick expense lines that belong to this cafÃÂ©. Use Adjustments for partial amounts.</p></div>
      <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
        <div style={{display:'flex',gap:'2px'}}>{[3,6,12].map(m=><button key={m} onClick={()=>{setMonths(m);load(m)}} style={{fontSize:'11px',padding:'3px 8px',borderRadius:'20px',cursor:'pointer',background:months===m?'var(--espresso)':'transparent',border:'1px solid '+(months===m?'var(--espresso)':'var(--border)'),color:months===m?'var(--crema-light)':'var(--text-secondary)'}}>{m}m</button>)}</div>
        <button onClick={save} disabled={saving} style={{fontSize:'12px',padding:'5px 14px',borderRadius:'7px',background:saved?'var(--success)':'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer'}}>{saving?'SavingÃ¢ÂÂ¦':saved?'Ã¢ÂÂ Saved':'Save mapping'}</button>
      </div>
    </div>
...
export default function CafeDashboard(){
  ...
  async function init(){
    ...
    if(sqC){setStep(1,'active','Checking cacheÃ¢ÂÂ¦');await loadSales(12,false);setStep(1,'done')}else setStep(1,'done','Not connected')
  ...
  return(<>
    <Head><title>{cafe?.name} Ã¢ÂÂ CafÃÂ©os</title></Head>
    <div style={{minHeight:'100vh',background:'var(--milk)'}}>
      <nav style={s.nav}>
        <div style={{display:'flex',alignItems:'center',gap:'14px'}}><Link href="/dashboard" style={{color:'var(--text-muted)',fontSize:'13px',textDecoration:'none'}}>Ã¢ÂÂ All cafÃÂ©s</Link><span style={s.logo}>Ã¢ÂÂ {cafe?.name}</span></div>
        <div style={{display:'flex',alignItems:'center',gap:'7px'}}>
          ...
          {sqConn?<button style={s.btnSm} onClick={()=>loadSales(settings.months,true)} disabled={syncing}>{syncing?'SyncingÃ¢ÂÂ¦':'Ã¢ÂÂ» Sync'}</button>:<button style={{...s.btnSm,background:'var(--espresso)',color:'var(--crema-light)',border:'none'}} onClick={connSq}>Connect Square</button>}
        </div>
      </nav>
      <main style={{maxWidth:'960px',margin:'0 auto',padding:'1.25rem 1.5rem 4rem'}}>
        {alerts.map((a,i)=><div key={i} style={{display:'flex',gap:'8px',padding:'8px 12px',borderRadius:'8px',border:'1px solid',marginBottom:'5px',fontSize:'12px',background:a.severity==='positive'?'var(--success-light)':a.severity==='critical'?'var(--danger-light)':'var(--warning-light)',borderColor:a.severity==='positive'?'var(--success)':a.severity==='critical'?'var(--danger)':'var(--warning)'}}><span>{a.severity==='positive'?'Ã°ÂÂÂ':a.severity==='critical'?'Ã°ÂÂÂ¨':'Ã¢ÂÂ Ã¯Â¸Â'}</span><div><strong>{a.title}</strong><span style={{color:'var(--text-secondary)',marginLeft:'5px'}}>{a.message}</span></div></div>)}
        ...
        {tab==='overview'&&!sqConn&&<div style={{textAlign:'center',padding:'3rem 2rem',background:'white',borderRadius:'14px',border:'1px solid var(--border)'}}><div style={{fontSize:'36px',marginBottom:'0.75rem'}}>Ã°ÂÂÂ</div><h2 style={{marginBottom:'6px',fontSize:'19px'}}>Connect Square POS</h2><p style={{color:'var(--text-secondary)',marginBottom:'1rem',fontSize:'13px'}}>Pull real sales data for an accurate valuation.</p><button style={{fontSize:'12px',padding:'6px 18px',borderRadius:'7px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer'}} onClick={connSq}>Connect with Square</button></div>}
        {tab==='overview'&&sqConn&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.875rem'}}><p style={{fontSize:'12px',color:'var(--text-muted)'}}>{syncing?'SyncingÃ¢ÂÂ¦':sd?(sd.orderCount||0).toLocaleString()+' orders':'LoadingÃ¢ÂÂ¦'}</p><div style={{display:'flex',gap:'3px'}}>{[3,6,12].map(m=><button key={m} onClick={()=>cp(m)} disabled={syncing} style={{fontSize:'11px',padding:'3px 9px',borderRadius:'20px',cursor:'pointer',background:settings.months===m?'var(--espresso)':'transparent',border:'1px solid '+(settings.months===m?'var(--espresso)':'var(--border-strong)'),color:settings.months===m?'var(--crema-light)':'var(--text-secondary)'}}>{m}m</button>)}</div></div>
          {val&&sd&&<>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'8px'}}>
              <div style={s.card}>
                <h3 style={s.ct}>Profit and loss{supplierCogs?.count>0&&<span style={{fontSize:'10px',fontWeight:400,color:'var(--success)',marginLeft:'5px'}}>COGS: {supplierCogs.count} suppliers Ã¢ÂÂ</span>}{!supplierCogs?.count&&xeroPL?.revenue>0&&<span style={{fontSize:'10px',fontWeight:400,color:'var(--success)',marginLeft:'5px'}}>via Xero Ã¢ÂÂ</span>}</h3>
                {[['Revenue',fmt(val.revenue),false,false],['COGS','Ã¢ÂÂ'+fmt(val.cogs),true,false],['Gross profit',fmt(val.grossProfit)+' ('+pct(val.grossMargin)+')',false,true],['Operating','Ã¢ÂÂ'+fmt(val.totalExpenses-val.cogs),true,false],['EBITDA',fmt(val.ebitda)+' ('+pct(val.ebitdaMargin)+')',false,true],['Add-backs','+ '+fmt(val.addBacks),false,false],['Adj. EBITDA',fmt(val.adjustedEbitda),false,true]].map((r,i)=><div key={i} style={{display:'flex',justifyContent:'space-between',padding:'3px 0',borderBottom:'1px solid var(--border)',fontWeight:r[3]?600:400}}><span style={{fontSize:'11px',color:'var(--text-secondary)'}}>{r[0]}</span><span style={{fontSize:'11px',color:r[2]?'var(--danger)':'inherit'}}>{r[1]}</span></div>)}
                ...
                {xeroConn&&!supplierCogs?.count&&<div style={{marginTop:'8px',padding:'5px 8px',background:'var(--warning-light)',borderRadius:'6px',fontSize:'10px',color:'var(--warning)'}}>Ã¢ÂÂ Go to Suppliers tab to map your cafÃÂ©Ã¢ÂÂs COGS suppliers.</div>}
              </div>
            ...
            {!xeroPL&&<div style={s.card}><h3 style={s.ct}>Adjust assumptions</h3><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'0.875rem'}}>{[['COGS %','Food, packaging',10,70,1,settings.cogsPercent,settings.cogsPercent+'%','cogsPercent'],['Operating %','Rent, wages',10,80,1,settings.opexPercent,settings.opexPercent+'%','opexPercent'],['Revenue x','0.3xÃ¢ÂÂ0.8x',0.1,2,0.05,settings.revenueMultiple,settings.revenueMultiple.toFixed(2)+'x','revenueMultiple'],['EBITDA x','2xÃ¢ÂÂ4x',0.5,8,0.25,settings.ebitdaMultiple,settings.ebitdaMultiple.toFixed(2)+'x','ebitdaMultiple']].map(r=><div key={r[7]}><div style={{display:'flex',justifyContent:'space-between',marginBottom:'2px'}}><span style={{fontSize:'11px',color:'var(--text-secondary)'}}>{r[0]}</span><strong style={{fontSize:'11px',fontFamily:'serif'}}>{r[6]}</strong></div><input type="range" min={r[2]} max={r[3]} step={r[4]} value={r[5]} onChange={e=>ss(r[7],parseFloat(e.target.value))} style={{width:'100%'}}/><p style={{fontSize:'10px',color:'var(--text-muted)',marginTop:'1px'}}>{r[1]}</p></div>)}</div></div>}
            {xeroPL?.revenue>0&&<div style={s.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.75rem'}}><h3 style={s.ct}>Revenue &amp; EBITDA multiples</h3><button onClick={()=>setTab('suppliers')} style={{fontSize:'10px',padding:'2px 8px',borderRadius:'6px',background:'transparent',border:'1px solid var(--border)',cursor:'pointer',color:'var(--text-muted)'}}>Edit suppliers Ã¢ÂÂ</button></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'0.875rem'}}>{[['Revenue x','0.3xÃ¢ÂÂ0.8x',0.1,2,0.05,settings.revenueMultiple,settings.revenueMultiple.toFixed(2)+'x','revenueMultiple'],['EBITDA x','2xÃ¢ÂÂ4x',0.5,8,0.25,settings.ebitdaMultiple,settings.ebitdaMultiple.toFixed(2)+'x','ebitdaMultiple']].map(r=><div key={r[7]}><div style={{display:'flex',justifyContent:'space-between',marginBottom:'2px'}}><span style={{fontSize:'11px',color:'var(--text-secondary)'}}>{r[0]}</span><strong style={{fontSize:'11px',fontFamily:'serif'}}>{r[6]}</strong></div><input type="range" min={r[2]} max={r[3]} step={r[4]} value={r[5]} onChange={e=>ss(r[7],parseFloat(e.target.value))} style={{width:'100%'}}/><p style={{fontSize:'10px',color:'var(--text-muted)',marginTop:'1px'}}>{r[1]}</p></div>)}</div></div>}
          </>}
        </>}
