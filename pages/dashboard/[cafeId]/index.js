...
const TODAY='2026-04-17',SK='sb-edoucarmulyjeqiydjxd-auth-token'
...
function itemValue(i){
  ...
  if(i.ownership==='roastery'||i.ownership==='leased')return 0
  if(m==='secondhand')return parseFloat(i.secondhand_value||0)
  if(m==='replacement'){const cp=i.condition==='excellent'?.85:i.condition==='good'?.65:i.condition==='fair'?.4:.2;return parseFloat(i.replacement_cost||0)*cp}
  if(m==='manual')return parseFloat(i.manual_value||0)
  if(!i.purchase_price||!i.purchase_date)return 0
  const y=(new Date()-new Date(i.purchase_date))/(365.25*24*3600*1000)
  return parseFloat(i.purchase_price)*Math.max(0,1-y/(i.depreciation_years||5))
}
...
function Pill({label,active,color,onClick}){
  ...
  return <button type="button" onClick={onClick} style={{padding:'3px 10px',borderRadius:'20px',border:'1px solid '+(active?c:'var(--border)'),background:active?c+'18':'white',color:active?c:'var(--text-secondary)',fontSize:'12px',cursor:'pointer',fontWeight:active?600:400,whiteSpace:'nowrap'}}>{label}</button>
}
function ItemForm({initial,onSave,onCancel,onDelete,saving,err,isEdit}){
  const[f,setF]=useState(initial||EF)
  const set=(k,v)=>setF(p=>({...p,[k]:v}))
  const mode=f.valuation_mode||'depreciated'
  ...
  return(
    <div style={{background:'white',border:'1px solid var(--crema)',borderRadius:'10px',padding:'1rem',marginBottom:'8px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'6px',marginBottom:'10px'}}>
        ...
        <div><label style={LB}>Purchased</label><input type="date" style={II} max="2026-04-17" value={f.purchase_date||''} onChange={e=>set('purchase_date',e.target.value)}/></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'8px'}}>
        <div>
          <label style={{...LB,marginBottom:'5px'}}>Ownership</label>
        ...
        <div>
          <label style={{...LB,marginBottom:'5px'}}>Valuation method</label>
          <div style={{display:'flex',gap:'4px',flexWrap:'wrap'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div style={{display:'flex',gap:'6px'}}>
          <button type="button" disabled={saving||!f.name} onClick={()=>onSave(f)} style={{fontSize:'12px',padding:'5px 14px',borderRadius:'7px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer',opacity:saving||!f.name?.6:1}}>{saving?'Saving…':'Save'}</button>
...
function EquipmentTab({cafeId,equipment,onRefresh}){
  ...
  const owned=equipment.filter(i=>i.ownership!=='roastery'&&i.ownership!=='leased')
  const excl=equipment.filter(i=>i.ownership==='roastery'||i.ownership==='leased')
  const total=owned.reduce((s,i)=>s+itemValue(i),0)
  ...
  const OB=item=>{
    if(item.ownership==='roastery')return <span style={{fontSize:'10px',padding:'1px 6px',borderRadius:'10px',background:'#fff3cd',color:'#856404',fontWeight:500}}>Roastery</span>
    if(item.ownership==='leased')return <span style={{fontSize:'10px',padding:'1px 6px',borderRadius:'10px',background:'var(--crema-pale)',color:'var(--text-muted)',fontWeight:500}}>Leased</span>
    return null
  }
    ...
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
        <div>
          ...
          <p style={{color:'var(--text-secondary)',fontSize:'13px'}}>Sale value: <strong style={{color:'var(--sage)'}}>{fmt(total)}</strong>{excl.length>0&&<span style={{color:'var(--text-muted)',fontWeight:400}}> · {excl.length} excluded</span>}</p>
        </div>
      ...
      {equipment.length>0&&<div style={{display:'grid',gap:'5px'}}>
        {equipment.map(item=>(
          <div key={item.id}>
            {editId===item.id
              ...
              :<div style={{background:'white',border:'1px solid var(--border)',borderRadius:'9px',padding:'0.65rem 0.875rem',display:'flex',justifyContent:'space-between',alignItems:'center',opacity:item.ownership==='roastery'||item.ownership==='leased'?.7:1}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:'11px',color:'var(--text-muted)'}}>
                    {item.purchase_price&&fmt(item.purchase_price)+' new'}
                    {item.purchase_date&&' · '+new Date(item.purchase_date).getFullYear()}
                    {item.notes&&' · '+item.notes.substring(0,45)+(item.notes.length>45?'…':'')}
                  </div>
                </div>
                <div style={{display:'flex',alignItems:'center',gap:'6px',marginLeft:'8px',flexShrink:0}}>
                  {item.ownership==='roastery'||item.ownership==='leased'
                    ...
                    :<div style={{textAlign:'right'}}><div style={{fontSize:'14px',fontWeight:600,fontFamily:'serif'}}>{fmt(itemValue(item))}</div><div style={{fontSize:'10px',color:'var(--text-muted)'}}>sale value</div></div>
                  }
...
function AdjustmentsTab({cafeId,adjustments,onRefresh}){
  ...
  const[saving,setSaving]=useState(false)
  const ab=adjustments.filter(a=>a.type==='add_back').reduce((s,a)=>s+Number(a.annual_amount),0)
  const rm=adjustments.filter(a=>a.type==='remove').reduce((s,a)=>s+Number(a.annual_amount),0)
  ...
  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
        <div><h2 style={{fontSize:'19px',marginBottom:'2px'}}>Owner adjustments</h2><p style={{color:'var(--text-secondary)',fontSize:'13px'}}>Normalise EBITDA for true earnings</p></div>
        <button onClick={()=>setShowForm(true)} style={{fontSize:'12px',padding:'5px 12px',borderRadius:'7px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer'}}>+ Add</button>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'1rem'}}>
        <div style={{background:'var(--success-light)',borderRadius:'9px',padding:'0.65rem 0.875rem'}}><p style={{fontSize:'10px',color:'var(--success)',fontWeight:600,marginBottom:'2px',textTransform:'uppercase',letterSpacing:'.04em'}}>Add-backs</p><p style={{fontSize:'18px',fontFamily:'serif',color:'var(--success)'}}>+{fmt(ab)}</p></div>
        <div style={{background:'var(--danger-light)',borderRadius:'9px',padding:'0.65rem 0.875rem'}}><p style={{fontSize:'10px',color:'var(--danger)',fontWeight:600,marginBottom:'2px',textTransform:'uppercase',letterSpacing:'.04em'}}>Removals</p><p style={{fontSize:'18px',fontFamily:'serif',color:'var(--danger)'}}>-{fmt(rm)}</p></div>
      </div>
      {showForm&&<div style={{background:'white',border:'1px solid var(--crema)',borderRadius:'10px',padding:'1rem',marginBottom:'8px'}}>
        <form onSubmit={save}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'8px'}}>
            <div><label style={LB}>Type</label><select style={SI} value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value}))}><option value="add_back">Add-back (increases EBITDA)</option><option value="remove">Remove (decreases EBITDA)</option></select></div>
            <div><label style={LB}>Label *</label><input style={II} value={form.label} onChange={e=>setForm(f=>({...f,label:e.target.value}))} placeholder="Owner salary" required/></div>
            <div style={{gridColumn:'1/-1'}}><label style={LB}>Description</label><input style={II} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="e.g. Personal car lease in expenses"/></div>
            <div><label style={LB}>Annual amount AUD *</label><input type="number" min="0" style={II} value={form.annual_amount} onChange={e=>setForm(f=>({...f,annual_amount:e.target.value}))} placeholder="24000" required/></div>
          </div>
          <div style={{display:'flex',gap:'6px'}}><button type="submit" disabled={saving} style={{fontSize:'12px',padding:'5px 14px',borderRadius:'7px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer'}}>{saving?'Saving…':'Save'}</button><button type="button" onClick={()=>setShowForm(false)} style={{fontSize:'12px',padding:'5px 10px',borderRadius:'7px',background:'transparent',color:'var(--text-secondary)',border:'1px solid var(--border)',cursor:'pointer'}}>Cancel</button></div>
        </form>
      </div>}
      {adjustments.length===0?<div style={{textAlign:'center',padding:'2.5rem',background:'white',borderRadius:'12px',border:'1px solid var(--border)'}}><div style={{fontSize:'32px',marginBottom:'8px'}}>💼</div><p style={{color:'var(--text-secondary)',fontSize:'13px'}}>No adjustments yet.</p></div>
        :<div style={{display:'grid',gap:'5px'}}>{adjustments.map(a=><div key={a.id} style={{background:'white',border:'1px solid var(--border)',borderRadius:'9px',padding:'0.65rem 0.875rem',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'1px'}}><strong style={{fontSize:'13px'}}>{a.label}</strong><span style={{fontSize:'10px',padding:'1px 6px',borderRadius:'10px',background:a.type==='add_back'?'var(--success-light)':'var(--danger-light)',color:a.type==='add_back'?'var(--success)':'var(--danger)',fontWeight:500}}>{a.type==='add_back'?'Add-back':'Remove'}</span></div>{a.description&&<p style={{fontSize:'11px',color:'var(--text-muted)'}}>{a.description}</p>}</div><div style={{display:'flex',alignItems:'center',gap:'8px'}}><span style={{fontSize:'13px',fontWeight:600,fontFamily:'serif',color:a.type==='add_back'?'var(--success)':'var(--danger)'}}>{a.type==='add_back'?'+':'-'}{fmt(Number(a.annual_amount))}/yr</span><button onClick={()=>del(a.id)} style={{background:'none',border:'none',color:'var(--danger)',cursor:'pointer',fontSize:'15px',padding:'0 2px'}}>✕</button></div></div>)}</div>}
    </div>
  )
            }
export default function CafeDashboard(){
  ...
  async function init(){
    ...
    setCafe(data)
    const sq=data.integrations?.find(i=>i.type==='square'),conn=sq?.status==='connected'
    setSqConn(conn)
    await Promise.all([loadEq(),loadAdj()])
    if(conn)await loadSales(12)
    setLoading(false)
  }
  async function loadSales(m2){
    ...
    if(res.ok){const d=await res.json();setSc(p=>({...p,[m]:d.salesData}));setSd(d.salesData)}
    setSyncing(false)
  }
  ...
  useEffect(()=>{
    if(!sd)return
    const r=sd.annualisedSales
    const exp=[{normalised_type:'cogs',amount:r*(settings.cogsPercent/100),is_excluded:false},{normalised_type:'opex',amount:r*(settings.opexPercent/100),is_excluded:false}]
    const v=calculateValuation(sd,exp,eq,adj,settings)
    ...
    <div style={{minHeight:'100vh',background:'var(--milk)'}}>
      <nav style={s.nav}>
        <div style={{display:'flex',alignItems:'center',gap:'14px'}}>
          <Link href="/dashboard" style={{color:'var(--text-muted)',fontSize:'13px',textDecoration:'none'}}>← All cafés</Link>
      ...
      <main style={{maxWidth:'920px',margin:'0 auto',padding:'1.25rem 1.5rem 4rem'}}>
        ...
        <div style={{display:'flex',gap:'3px',marginBottom:'1.125rem',background:'white',borderRadius:'9px',padding:'3px',border:'1px solid var(--border)'}}>
        ...
        {tab==='overview'&&sqConn&&<>
          ...
          {!sd&&!syncing&&<div style={{textAlign:'center',padding:'2rem',background:'white',borderRadius:'10px',border:'1px solid var(--border)',marginBottom:'0.875rem'}}><p style={{color:'var(--text-muted)',fontSize:'13px'}}>Click ↻ Sync to load data</p></div>}
          {val&&sd&&<>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'8px'}}>
              ...
              {hs&&<div style={s.card}>
                <h3 style={s.ct}>Business health</h3>
                <div style={{textAlign:'center',padding:'0.4rem 0 0.875rem'}}>
import{useRouter}from 'next/router'
import Head from 'next/head'
import Link from 'next/link'
import{supabase}from '../../../lib/supabase'
...
export default function CafeDashboard(){
  ...
  const[loading,setLoading]=useState(true),[syncing,setSyncing]=useState(false),[tab,setTab]=useState('overview'),[sqConn,setSqConn]=useState(false)
  useEffect(()=>{if(cafeId)init()},[cafeId])
  async function init(){const{data:{user}}=await supabase.auth.getUser();if(!user){router.push('/login');return};const{data}=await supabase.from('cafes').select('*,integrations(*)').eq('id',cafeId).eq('owner_id',user.id).single();if(!data){router.push('/dashboard');return};setCafe(data);const sq=data.integrations?.find(i=>i.type==='square'),conn=sq?.status==='connected';setSqConn(conn);await Promise.all([loadEq(),loadAdj()]);if(conn)await loadSales(12);setLoading(false)}
  ...
  async function loadEq(){const res=await fetch('/api/cafes/'+cafeId+'/equipment',{headers:{Authorization:'Bearer '+getToken()}});if(res.ok){const d=await res.json();setEq(d.equipment||[])}}
  async function loadAdj(){const res=await fetch('/api/cafes/'+cafeId+'/adjustments',{headers:{Authorization:'Bearer '+getToken()}});if(res.ok){const d=await res.json();setAdj(d.adjustments||[])}}
  useEffect(()=>{if(!sd)return;const r=sd.annualisedSales;const exp=[{normalised_type:'cogs',amount:r*(settings.cogsPercent/100),is_excluded:false},{normalised_type:'opex',amount:r*(settings.opexPercent/100),is_excluded:false}];const v=calculateValuation(sd,exp,eq,adj,settings);setVal(v);setHs(calculateHealthScore(sd,v,[]));setAlerts(generateAlerts(sd,v))},[sd,eq,adj,settings])
  const ss=(k,v)=>setSettings(p=>({...p,[k]:v}))
  async function cp(m){ss('months',m);if(sc[m]){setSd(sc[m]);return};await loadSales(m)}
  async function forceSync(){setSc({});setSd(null);await loadSales(settings.months)}
  async function connSq(){const r=await fetch('/api/square/auth?cafeId='+cafeId);const{url}=await r.json();window.location.href=url}
  ...
  return(<>
    ...
    <div style={{minHeight:'100vh',background:'var(--milk)'}}>
      ...
      <main style={{maxWidth:'920px',margin:'0 auto',padding:'1.25rem 1.5rem 4rem'}}>
        ...
        {tab==='integrations'&&<div><h2 style={{fontSize:'18px',marginBottom:'0.875rem'}}>Integrations</h2>{[{id:'square',name:'Square',desc:'POS sales & orders',icon:'■',av:true,conn:sqConn},{id:'xero',name:'Xero',desc:'Accounting',icon:'✕',av:false},{id:'qb',name:'QuickBooks',desc:'Accounting',icon:'◆',av:false},{id:'ls',name:'Lightspeed',desc:'POS',icon:'⚡',av:false}].map(it=><div key={it.id} style={{background:'white',border:'1px solid '+(it.conn?'var(--success)':'var(--border)'),borderRadius:'9px',padding:'0.75rem',display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'5px'}}><div style={{display:'flex',alignItems:'center',gap:'9px'}}><div style={{width:'32px',height:'32px',borderRadius:'7px',background:'var(--crema-pale)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px'}}>{it.icon}</div><div><div style={{display:'flex',alignItems:'center',gap:'5px'}}><strong style={{fontSize:'12px'}}>{it.name}</strong>{it.conn&&<span style={{fontSize:'9px',padding:'1px 5px',borderRadius:'8px',background:'var(--success-light)',color:'var(--success)',fontWeight:600}}>Connected</span>}{!it.av&&!it.conn&&<span style={{fontSize:'9px',padding:'1px 5px',borderRadius:'8px',background:'var(--crema-pale)',color:'var(--text-muted)'}}>Coming soon</span>}</div><p style={{fontSize:'10px',color:'var(--text-muted)',marginTop:'1px'}}>{it.desc}</p></div></div>{it.av&&!it.conn&&<button style={{fontSize:'11px',padding:'4px 10px',borderRadius:'6px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer'}} onClick={connSq}>Connect</button>}{it.conn&&<span style={{fontSize:'11px',color:'var(--success)'}}>✓ Active</span>}</div>)}</div>}
      </main>
    </div>
  </>)
}
