...
function WagesMappingTab({cafeId,onSaved}){
  ...
  React.useEffect(()=>{
    ...
    .then(r=>r.json()).then(d=>{if(d.error){setErr(d.error)}else{setStaff(d.staff||[]);setMapping(d.mapping||{})}setLoading(false)}).catch(e=>{setErr(e.message);setLoading(false)})
  },[cafeId])
  const toggle=name=>{const u={...mapping};u[name]?delete u[name]:u[name]=true;setMapping(u)}
  const save=async()=>{
    setSaving(true)
    const r=await fetch('/api/xero/wages?cafeId='+cafeId,{method:'POST',headers:{Authorization:'Bearer '+tok(),'Content-Type':'application/json'},body:JSON.stringify({mapping})})
    const d=await r.json();setSaving(false)
    if(d.error){setErr(d.error)}else{onSaved&&onSaved(mapping,staff)}
  }
  const filtered=staff.filter(s=>!search||s.name.toLowerCase().includes(search.toLowerCase()))
  const selected=staff.filter(s=>mapping[s.name])
  const wageCogs=selected.reduce((t,s)=>t+(s.total||0),0)
  const fmt=n=>'{cafeId,onSaved}){
...
+(n||0).toLocaleString('en-AU',{minimumFractionDigits:0,maximumFractionDigits:0})
  if(loading) return React.createElement('div',{style:{padding:'2rem',textAlign:'center',color:'var(--text-muted)',fontSize:'13px'}},'Loading staff from Xero...')
  if(err) return React.createElement('div',{style:{padding:'2rem',color:'var(--danger)',fontSize:'13px'}},'Error: '+err)
  return React.createElement('div',{style:{padding:'0.5rem 0'}},
    React.createElement('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'0.75rem'}},
      React.createElement('div',null,
        React.createElement('h3',{style:{fontSize:'14px',fontWeight:600,marginBottom:'2px'}},'Caf\u00e9 Wages'),
        React.createElement('p',{style:{fontSize:'11px',color:'var(--text-muted)'}},'Tick staff who work at this caf\u00e9. Their total payments will be included as a wages expense.')
      ),
      React.createElement('div',{style:{textAlign:'right'}},
        React.createElement('div',{style:{fontSize:'18px',fontWeight:700,color:'var(--espresso)'}},[fmt(wageCogs)]),
        React.createElement('div',{style:{fontSize:'10px',color:'var(--text-muted)'}},selected.length+' of '+staff.length+' staff selected')
      )
    ),
    React.createElement('input',{placeholder:'Search staff...',value:search,onChange:e=>setSearch(e.target.value),style:{width:'100%',padding:'6px 10px',borderRadius:'7px',border:'1px solid var(--border)',fontSize:'12px',marginBottom:'0.5rem',boxSizing:'border-box'}}),
    React.createElement('div',{style:{maxHeight:'380px',overflowY:'auto',border:'1px solid var(--border)',borderRadius:'9px',marginBottom:'0.75rem'}},
      filtered.length===0
        ? React.createElement('div',{style:{padding:'2rem',textAlign:'center',color:'var(--text-muted)',fontSize:'12px'}},'No staff found')
        : filtered.map((s,i)=>{
            const sel=!!mapping[s.name]
            return React.createElement('div',{key:s.contactId||s.name,onClick:()=>toggle(s.name),style:{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 12px',cursor:'pointer',borderBottom:i<filtered.length-1?'1px solid var(--border)':'none',background:sel?'var(--success-light)':'white',transition:'background 0.15s'}},
              React.createElement('div',{style:{display:'flex',alignItems:'center',gap:'10px'}},
                React.createElement('div',{style:{width:'16px',height:'16px',borderRadius:'4px',border:'2px solid '+(sel?'var(--success)':'var(--border)'),background:sel?'var(--success)':'white',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}},
                  sel&&React.createElement('span',{style:{color:'white',fontSize:'10px',lineHeight:1}},'\u2713')
                ),
                React.createElement('span',{style:{fontSize:'12px',fontWeight:sel?600:400}},[s.name])
              ),
              React.createElement('div',{style:{textAlign:'right'}},
                React.createElement('div',{style:{fontSize:'12px',fontWeight:600}},[fmt(s.total)]),
                React.createElement('div',{style:{fontSize:'10px',color:'var(--text-muted)'}},[s.count]+' payment'+(s.count!==1?'s':''))
              )
            )
          })
    ),
    React.createElement('button',{onClick:save,disabled:saving,style:{width:'100%',padding:'8px',borderRadius:'8px',background:'var(--espresso)',color:'white',border:'none',fontSize:'13px',fontWeight:600,cursor:'pointer'}},saving?'Saving...':'Save caf\u00e9 wages')
  )
}
