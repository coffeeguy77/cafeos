...
function ItemForm({initial,onSave,onCancel,onDelete,saving,err,isEdit}){
  ...
  return(<div style={{background:'white',border:'1px solid var(--crema)',borderRadius:'10px',padding:'1rem',marginBottom:'8px'}}>
    ...
    <div style={{marginBottom:'10px'}}><label style={LB}>Notes</label><input style={II} value={f.notes||''} onChange={e=>set('notes',e.target.value)} placeholder="e.g. Negotiated as not in sale"/></div>
...
const SAMPLE_CSV=`name,category,brand,purchase_date,purchase_price,condition,depreciation_years,valuation_mode,secondhand_value,replacement_cost,manual_value,ownership,notes
La Marzocca Linea PB 2 Group,espresso_machine,La Marzocca,2022-03-15,12000,good,10,depreciated,,,, cafe,Main bar machine
Anfim SP2 Grinder,grinder,Anfim,2022-03-15,2200,good,7,secondhand,1800,,,cafe,
Mahlkonig EK43,grinder,Mahlkonig,2021-06-01,3500,fair,7,depreciated,,,,cafe,Filter grinder
True Refrigerator TUC-27F,refrigeration,True,2020-01-10,4800,good,10,depreciated,,,,cafe,
...
function downloadSampleCSV(){
  ...
  a.href=url;a.download='equipment_import_template.csv';a.click();
...
function parseCSV(text){
  ...
  return lines.slice(1).filter(l=>l.trim()).map(line=>{
    ...
    vals.push(cur.trim());
    const obj={};
    headers.forEach((h,i)=>{let v=(vals[i]||'').replace(/^"|"$/g,'').trim();if(v)obj[h]=v});
    return obj;
  });
}
...
function EquipmentTab({cafeId,equipment,onRefresh}){
  ...
  if(filterOwn!=='all') filtered=filtered.filter(i=>i.ownership===filterOwn||(!i.ownership&&filterOwn==='cafe'))
  filtered.sort((a,b)=>{
    if(sortBy==='value_desc') return b._val-a._val
    if(sortBy==='value_asc') return a._val-b._val
    if(sortBy==='name_asc') return a.name.localeCompare(b.name)
    if(sortBy==='name_desc') return b.name.localeCompare(a.name)
    if(sortBy==='date_desc') return new Date(b.purchase_date||0)-new Date(a.purchase_date||0)
    if(sortBy==='cat') return (a.category||'').localeCompare(b.category||'')
    return 0
  })
  // Suspended always at bottom
  const active=filtered.filter(i=>!i.suspended),susp=filtered.filter(i=>i.suspended)
  const display=[...active,...susp]

  // Autocomplete
  function onSearch(v){
    setSearch(v)
    if(v.length>1){const s=equipment.filter(i=>i.name.toLowerCase().includes(v.toLowerCase())).map(i=>i.name).slice(0,6);setSuggestions(s);setShowSugg(s.length>0)}else{setShowSugg(false)}
  }
  ...
  function onFileChange(e){
    ...
    reader.onload=ev=>{
      try{const rows=parseCSV(ev.target.result);setCsvPreview(rows);setImportErr('')}
      catch(ex){setImportErr('Could not parse CSV: '+ex.message)}
    };reader.readAsText(file);
  }
  async function doImport(){
    if(!csvPreview?.length)return;
    setImporting(true);setImportErr('')
    const res=await fetch('/api/cafes/'+cafeId+'/equipment',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+getToken()},body:JSON.stringify(csvPreview)})
    const d=await res.json()
    if(!res.ok){setImportErr(d.error||'Import failed');setImporting(false);return}
    await onRefresh();setShowImport(false);setCsvPreview(null);setImporting(false)
  }

  async function toggleSuspend(item){
    await fetch('/api/cafes/'+cafeId+'/equipment',{method:'PATCH',headers:{'Content-Type':'application/json',Authorization:'Bearer '+getToken()},body:JSON.stringify({id:item.id,suspended:!item.suspended})})
    await onRefresh()
  }
  ...
  return(<div>
    ...
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'0.875rem'}}>
      <div>
        <h2 style={{fontSize:'19px',marginBottom:'2px'}}>Equipment ledger</h2>
        <div style={{display:'flex',gap:'12px',flexWrap:'wrap'}}>
          <span style={{fontSize:'12px',color:'var(--sage)',fontWeight:600}}>{fmt(total)} sale value</span>
          {suspended.length>0&&<span style={{fontSize:'12px',color:'#d97706',fontWeight:500}}>{suspended.length} suspended ({fmt(suspended.reduce((s,i)=>s+itemValue(i),0))})</span>}
          <span style={{fontSize:'12px',color:'var(--text-muted)'}}>{owned.length} active items</span>
        </div>
      </div>
      <div style={{display:'flex',gap:'6px'}}>
        <button onClick={()=>setShowImport(!showImport)} style={{fontSize:'11px',padding:'4px 10px',borderRadius:'7px',background:'white',color:'var(--text-secondary)',border:'1px solid var(--border)',cursor:'pointer'}}>⬆ Import CSV</button>
    ...
    {showImport&&<div style={{background:'white',border:'1px solid var(--border)',borderRadius:'12px',padding:'1rem',marginBottom:'1rem'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
        <h3 style={{fontSize:'13px',fontWeight:600,color:'var(--espresso)'}}>Bulk import from CSV</h3>
        <button onClick={downloadSampleCSV} style={{fontSize:'11px',padding:'3px 9px',borderRadius:'6px',background:'transparent',border:'1px solid var(--border)',cursor:'pointer',color:'var(--text-muted)'}}>⬇ Download template</button>
      </div>
      <p style={{fontSize:'11px',color:'var(--text-muted)',marginBottom:'10px'}}>Download the template, fill it in, then upload. Columns: name, category, brand, purchase_date (YYYY-MM-DD), purchase_price, condition, depreciation_years, valuation_mode, secondhand_value, replacement_cost, manual_value, ownership, notes.</p>
      <div style={{border:'2px dashed var(--border)',borderRadius:'8px',padding:'1.5rem',textAlign:'center',marginBottom:'10px'}}>
        <input type="file" accept=".csv" onChange={onFileChange} style={{display:'none'}} id="csv-upload"/>
        <label htmlFor="csv-upload" style={{cursor:'pointer',fontSize:'12px',color:'var(--espresso)',fontWeight:500}}>📂 Choose CSV file</label>
        {csvPreview&&<p style={{fontSize:'11px',color:'var(--success)',marginTop:'5px'}}>{csvPreview.length} rows ready to import</p>}
      </div>
      ...
      {csvPreview&&csvPreview.length>0&&<div style={{marginBottom:'10px',maxHeight:'150px',overflowY:'auto',border:'1px solid var(--border)',borderRadius:'6px'}}>
        <table style={{width:'100%',fontSize:'10px',borderCollapse:'collapse'}}>
          <thead><tr style={{background:'var(--crema-pale)'}}>{Object.keys(csvPreview[0]).map(h=><th key={h} style={{padding:'3px 6px',textAlign:'left',color:'var(--text-muted)',fontWeight:600,borderBottom:'1px solid var(--border)'}}>{h}</th>)}</tr></thead>
          <tbody>{csvPreview.slice(0,5).map((row,i)=><tr key={i} style={{borderBottom:'1px solid var(--border)'}}>{Object.values(row).map((v,j)=><td key={j} style={{padding:'3px 6px',color:'var(--text-secondary)'}}>{String(v).substring(0,25)}</td>)}</tr>)}</tbody>
        </table>
        {csvPreview.length>5&&<p style={{fontSize:'10px',color:'var(--text-muted)',padding:'3px 6px'}}>...and {csvPreview.length-5} more rows</p>}
      </div>}
      <div style={{display:'flex',gap:'6px'}}>
        <button onClick={doImport} disabled={importing||!csvPreview?.length} style={{fontSize:'12px',padding:'5px 14px',borderRadius:'7px',background:'var(--espresso)',color:'var(--crema-light)',border:'none',cursor:'pointer',opacity:importing||!csvPreview?.length?0.6:1}}>{importing?'Importing…':'Import '+((csvPreview?.length)||0)+' items'}</button>
        <button onClick={()=>{setShowImport(false);setCsvPreview(null);setImportErr('')}} style={{fontSize:'12px',padding:'5px 10px',borderRadius:'7px',background:'transparent',color:'var(--text-secondary)',border:'1px solid var(--border)',cursor:'pointer'}}>Cancel</button>
      </div>
    </div>}
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr auto auto auto',gap:'6px',marginBottom:'10px',alignItems:'center',position:'relative'}}>
      <div style={{position:'relative'}}>
        <input style={{...II,paddingLeft:'28px'}} placeholder="Search equipment…" value={search} onChange={e=>onSearch(e.target.value)} onBlur={()=>setTimeout(()=>setShowSugg(false),150)}/>
        <span style={{position:'absolute',left:'8px',top:'50%',transform:'translateY(-50%)',fontSize:'12px',color:'var(--text-muted)'}}>&#128269;</span>
        {showSugg&&<div style={{position:'absolute',top:'100%',left:0,right:0,background:'white',border:'1px solid var(--border)',borderRadius:'6px',zIndex:10,boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}}>
          {suggestions.map(s=><div key={s} onMouseDown={()=>{setSearch(s);setShowSugg(false)}} style={{padding:'6px 10px',fontSize:'12px',cursor:'pointer',color:'var(--espresso)'}} onMouseEnter={e=>e.target.style.background='var(--crema-pale)'} onMouseLeave={e=>e.target.style.background='white'}>{s}</div>)}
        </div>}
      </div>
      <select style={SI} value={filterCat} onChange={e=>setFilterCat(e.target.value)}>
        <option value="all">All categories</option>
        {CATS.map(c=><option key={c} value={c}>{c.replace(/_/g,' ')}</option>)}
      </select>
      <select style={SI} value={filterOwn} onChange={e=>setFilterOwn(e.target.value)}>
        <option value="all">All ownership</option>
        <option value="cafe">Cafe owned</option>
        <option value="roastery">Roastery</option>
        <option value="leased">Leased</option>
      </select>
      <select style={SI} value={sortBy} onChange={e=>setSortBy(e.target.value)}>
    ...
    <div style={{display:'grid',gap:'5px'}}>
      {display.map(item=><div key={item.id}>
        {editId===item.id
          ...
          :<div style={{background:item.suspended?'#fffbeb':'white',border:'1px solid '+(item.suspended?'#f59e0b':'var(--border)'),borderRadius:'9px',padding:'0.65rem 0.875rem',display:'flex',justifyContent:'space-between',alignItems:'center',opacity:item.ownership==='roastery'||item.ownership==='leased'?0.7:1}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:'flex',alignItems:'center',gap:'5px',marginBottom:'1px',flexWrap:'wrap'}}>
                <strong style={{fontSize:'13px',color:item.suspended?'#92400e':'inherit'}}>{item.name}</strong>
                {item.brand&&<span style={{fontSize:'11px',color:'var(--text-muted)'}}>{item.brand}</span>}
                {OB(item)}
                {item.suspended&&<span style={{fontSize:'10px',padding:'1px 7px',borderRadius:'10px',background:'#fef3c7',color:'#92400e',fontWeight:600}}>⏸ Suspended from sale</span>}
                <span style={{fontSize:'10px',padding:'1px 6px',borderRadius:'10px',background:'var(--crema-pale)',color:'var(--text-muted)'}}>{(item.category||'').replace(/_/g,' ')}</span>
              </div>
              <div style={{fontSize:'11px',color:'var(--text-muted)'}}>{item.purchase_price&&fmt(item.purchase_price)+' new'}{item.purchase_date&&' · '+new Date(item.purchase_date).getFullYear()}{item.notes&&' · '+item.notes.substring(0,50)+(item.notes.length>50?'…':'')}</div>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:'6px',marginLeft:'8px',flexShrink:0}}>
              {item.suspended
                ?<div style={{textAlign:'right'}}><div style={{fontSize:'12px',color:'#92400e',fontStyle:'italic'}}>suspended</div><div style={{fontSize:'10px',color:'#b45309'}}>{fmt(itemValue(item))} excluded</div></div>
                :item.ownership==='roastery'||item.ownership==='leased'
                  ?<span style={{fontSize:'11px',color:'var(--text-muted)',fontStyle:'italic'}}>not included</span>
                  :<div style={{textAlign:'right'}}><div style={{fontSize:'14px',fontWeight:600,fontFamily:'serif'}}>{fmt(item._val)}</div><div style={{fontSize:'10px',color:'var(--text-muted)'}}>sale value</div></div>}
              <button title={item.suspended?'Re-include in sale':'Suspend from sale'} onClick={()=>toggleSuspend(item)} style={{background:'none',border:'1px solid '+(item.suspended?'#f59e0b':'var(--border)'),borderRadius:'5px',padding:'2px 6px',cursor:'pointer',fontSize:'11px',color:item.suspended?'#92400e':'var(--text-muted)',lineHeight:1}}>{item.suspended?'▶':'⏸'}</button>
              <button title="Duplicate" onClick={()=>dup(item)} style={{background:'none',border:'1px solid var(--border)',borderRadius:'5px',padding:'2px 6px',cursor:'pointer',fontSize:'12px',color:'var(--text-muted)',lineHeight:1}}>⧉</button>
              <button title="Edit" onClick={()=>{setEditId(item.id);setShowAdd(false);setErr('')}} style={{background:'none',border:'1px solid var(--border)',borderRadius:'5px',padding:'2px 6px',cursor:'pointer',fontSize:'12px',color:'var(--text-secondary)',lineHeight:1}}>✎</button>
            </div>
          </div>}
      </div>)}
    </div>
...
function AdjustmentsTab({cafeId,adjustments,onRefresh,xeroLines,xeroConn}){
  ...
  function resolve(adj,xLine){if(adj.source==='xero'&&xLine){if(adj.method==='percentage')return xLine*(parseFloat(adj.percentage||0)/100);if(adj.method==='fixed')return parseFloat(adj.annual_amount||0);return xLine}if(adj.method==='percentage'&&adj.source==='manual')return parseFloat(adj.annual_amount||0)*(parseFloat(adj.percentage||0)/100);return parseFloat(adj.annual_amount||0)}
  ...
  return(<div>
    ...
    {showForm&&<div style={{background:'white',border:'1px solid var(--crema)',borderRadius:'12px',padding:'1.25rem',marginBottom:'1rem'}}>
      ...
      <form onSubmit={save}>
        ...
        <div style={{marginBottom:'12px'}}><label style={LB}>Notes</label><input style={II} value={form.description} onChange={e=>sf('description',e.target.value)} placeholder="e.g. 30% of shared rent — café occupies ground floor only"/></div>
...
function SupplierMappingTab({cafeId,onSaved}){
  ...
  const cafeS=suppliers.filter(s=>mapping[s.name]),cafeTotal=cafeS.reduce((s,x)=>s+x.total,0),allTotal=suppliers.reduce((s,x)=>s+x.total,0)
  const filtered=search?suppliers.filter(s=>s.name.toLowerCase().includes(search.toLowerCase())):suppliers
  if(loading) return <div style={{textAlign:'center',padding:'3rem',color:'var(--text-muted)',fontSize:'13px'}}>Loading suppliers from Xero…</div>
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1rem'}}>
      <div><h2 style={{fontSize:'19px',marginBottom:'3px'}}>Supplier COGS mapping</h2><p style={{fontSize:'12px',color:'var(--text-muted)'}}>Tick the suppliers that sell to this café. Their total spend becomes your true COGS.</p></div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'8px',marginBottom:'1rem'}}>
      <div style={{background:'var(--success-light)',borderRadius:'9px',padding:'0.65rem 0.875rem',gridColumn:'span 2'}}><p style={{fontSize:'10px',color:'var(--success)',fontWeight:600,marginBottom:'2px',textTransform:'uppercase'}}>Café COGS (selected)</p><p style={{fontSize:'22px',fontFamily:'serif',color:'var(--success)'}}>{fmt(cafeTotal)}</p><p style={{fontSize:'10px',color:'var(--success)',marginTop:'1px'}}>{cafeS.length} suppliers · {allTotal>0?Math.round(cafeTotal/allTotal*100):0}% of total</p></div>
      <div style={{background:'var(--crema-pale)',borderRadius:'9px',padding:'0.65rem 0.875rem'}}><p style={{fontSize:'10px',color:'var(--text-muted)',fontWeight:600,marginBottom:'2px',textTransform:'uppercase'}}>Total spend</p><p style={{fontSize:'18px',fontFamily:'serif',color:'var(--text-secondary)'}}>{fmt(allTotal)}</p></div>
      <div style={{background:'var(--crema-pale)',borderRadius:'9px',padding:'0.65rem 0.875rem'}}><p style={{fontSize:'10px',color:'var(--text-muted)',fontWeight:600,marginBottom:'2px',textTransform:'uppercase'}}>Annualised COGS</p><p style={{fontSize:'18px',fontFamily:'serif',color:'var(--espresso)'}}>{fmt(cafeTotal/months*12)}</p></div>
    </div>
    <div style={{display:'flex',gap:'8px',marginBottom:'10px',alignItems:'center'}}>
      <input style={{...II,flex:1}} placeholder="Search suppliers…" value={search} onChange={e=>setSearch(e.target.value)}/>
      <button onClick={()=>setAll(true)} style={{fontSize:'11px',padding:'4px 10px',borderRadius:'6px',border:'1px solid var(--border)',background:'white',cursor:'pointer',color:'var(--text-muted)',whiteSpace:'nowrap'}}>All</button>
      <button onClick={()=>setAll(false)} style={{fontSize:'11px',padding:'4px 10px',borderRadius:'6px',border:'1px solid var(--border)',background:'white',cursor:'pointer',color:'var(--text-muted)',whiteSpace:'nowrap'}}>None</button>
    </div>
    {suppliers.length===0&&<div style={{textAlign:'center',padding:'2.5rem',background:'white',borderRadius:'12px',border:'1px solid var(--border)'}}><div style={{fontSize:'32px',marginBottom:'8px'}}>📦</div><p style={{color:'var(--text-secondary)',fontSize:'13px'}}>No supplier data found in Xero for this period.</p></div>}
    <div style={{display:'grid',gap:'4px'}}>
      {filtered.filter(s=>mapping[s.name]).length>0&&<div style={{fontSize:'10px',color:'var(--success)',fontWeight:600,padding:'4px 2px',textTransform:'uppercase',letterSpacing:'.05em'}}>Selected as café COGS suppliers</div>}
      {filtered.filter(s=>mapping[s.name]).map(s=>{const p=allTotal>0?Math.round(s.total/allTotal*100):0;return(<div key={s.name} onClick={()=>toggle(s.name)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 12px',borderRadius:'8px',border:'1px solid var(--success)',background:'#f0faf4',cursor:'pointer',marginBottom:'2px'}}><div style={{width:'18px',height:'18px',borderRadius:'4px',border:'2px solid var(--success)',background:'var(--success)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{color:'white',fontSize:'11px',fontWeight:700}}>✓</span></div><span style={{flex:1,fontSize:'13px',fontWeight:600,color:'var(--espresso)'}}>{s.name}</span><div style={{display:'flex',alignItems:'center',gap:'10px'}}>{s.total>0&&<div style={{width:'80px',height:'4px',background:'var(--crema-pale)',borderRadius:'2px'}}><div style={{height:'100%',width:p+'%',background:'var(--success)',borderRadius:'2px',minWidth:'2px'}}/></div>}<span style={{fontSize:'13px',fontFamily:'serif',color:'var(--espresso)',width:'90px',textAlign:'right',fontWeight:600}}>{s.total>0?fmt(s.total):'—'}</span>{s.total>0&&<span style={{fontSize:'10px',color:'var(--text-muted)',width:'32px',textAlign:'right'}}>{p}%</span>}</div></div>)})}
      ...
      {filtered.filter(s=>!mapping[s.name]).map(s=>{const p=allTotal>0?Math.round(s.total/allTotal*100):0;return(<div key={s.name} onClick={()=>toggle(s.name)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 12px',borderRadius:'8px',border:'1px solid var(--border)',background:'white',cursor:'pointer',marginBottom:'2px'}}><div style={{width:'18px',height:'18px',borderRadius:'4px',border:'2px solid var(--border)',background:'white',flexShrink:0}}/><span style={{flex:1,fontSize:'13px',fontWeight:400,color:'var(--text-secondary)'}}>{s.name}</span><div style={{display:'flex',alignItems:'center',gap:'10px'}}>{s.total>0&&<div style={{width:'80px',height:'4px',background:'var(--crema-pale)',borderRadius:'2px'}}><div style={{height:'100%',width:p+'%',background:'var(--border)',borderRadius:'2px',minWidth:s.total>0?'2px':'0'}}/></div>}<span style={{fontSize:'13px',fontFamily:'serif',color:'var(--text-muted)',width:'90px',textAlign:'right'}}>{s.total>0?fmt(s.total):'—'}</span>{s.total>0&&<span style={{fontSize:'10px',color:'var(--text-muted)',width:'32px',textAlign:'right'}}>{p}%</span>}</div></div>)})}
    </div>
  </div>)
}
function XeroMappingTab({cafeId,onMappingSaved}){
  ...
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1rem'}}>
      <div><h2 style={{fontSize:'19px',marginBottom:'3px'}}>Xero P&L mapping</h2><p style={{fontSize:'12px',color:'var(--text-muted)'}}>Tick expense lines that belong to this café.</p></div>
      <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
        <div style={{display:'flex',gap:'2px'}}>{[3,6,12].map(m=><button key={m} onClick={()=>{setMonths(m);load(m)}} style={{fontSize:'11px',padding:'3px 8px',borderRadius:'20px',cursor:'pointer',background:months===m?'var(--espresso)':'transparent',border:'1px solid '+(months===m?'var(--espresso)':'var(--border)'),color:months===m?'var(--crema-light)':'var(--text-secondary)'}}>{m}m</button>)}</div>
...
export default function CafeDashboard(){
  ...
  const[xeroLines,setXeroLines]=useState([]),[supplierCogs,setSupplierCogs]=useState(null)
  ...
  async function loadXero(months){
    ...
    if(rawRes.ok){const rd=await rawRes.json();setXeroLines(rd.raw?.sections?.flatMap(s=>s.rows)||[])}
    const supRes=await fetch('/api/xero/suppliers?cafeId='+cafeId+'&months='+(months||settings.months),{headers:{Authorization:'Bearer '+getToken()}})
    if(supRes.ok){const sd2=await supRes.json();const mapped=sd2.suppliers?.filter(s=>sd2.mapping?.[s.name]);const total=mapped?.reduce((s,x)=>s+x.total,0)||0;if(mapped?.length>0)setSupplierCogs({total,months:months||settings.months,count:mapped.length})}
  ...
  const tabs=['overview','equipment','adjustments',xeroConn?'suppliers':null,xeroConn?'xero mapping':null,'integrations'].filter(Boolean)
          ...
          {val&&sd&&<>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'8px'}}>
              <div style={s.card}>
                ...
                {xeroConn&&!supplierCogs?.count&&<div style={{marginTop:'8px',padding:'5px 8px',background:'var(--warning-light)',borderRadius:'6px',fontSize:'10px',color:'var(--warning)'}}>→ Go to Suppliers tab to set your café COGS suppliers.</div>}
              </div>
