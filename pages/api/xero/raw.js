...
export default async function handler(req,res){
  ...
  try{
    const raw=await getRawProfitAndLoss(accessToken,ig.metadata?.tenant_id,parseInt(months))
    // Also load saved mapping if exists
    const{data:mappingRow}=await supabaseAdmin.from('xero_mappings').select('mapping').eq('cafe_id',cafeId).single()
    return res.status(200).json({raw,mapping:mappingRow?.mapping||null})
  }catch(e){
