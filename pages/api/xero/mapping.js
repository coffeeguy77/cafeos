...
export default async function handler(req,res){
  ...
  if(req.method==='GET'){
    const{data}=await supabaseAdmin.from('xero_mappings').select('mapping').eq('cafe_id',cafeId).single()
    return res.status(200).json({mapping:data?.mapping||{}})
  }

  if(req.method==='POST'){
    const{mapping}=req.body
    await supabaseAdmin.from('xero_mappings').upsert({cafe_id:cafeId,mapping},{onConflict:'cafe_id'})
