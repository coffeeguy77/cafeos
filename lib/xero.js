...
const SCOPES='openid profile email accounting.reports.profitandloss.read offline_access'
...
async function fetchPL(accessToken,tenantId,months){
  ...
  if(!res.ok){const e=await res.text();throw new Error('P&L fetch failed: '+res.status+' '+e)}
  return res.json()
}

// Returns raw sections with all rows for the mapping UI
export async function getRawProfitAndLoss(accessToken,tenantId,months){
  const data=await fetchPL(accessToken,tenantId,months)
  const report=data.Reports?.[0]
  if(!report) return null
  const sections=[]
  const numVal=v=>v?Math.abs(parseFloat(v.replace(/[^0-9.-]/g,''))||0):0
  for(const sec of report.Rows||[]){
    ...
    for(const row of sec.Rows||[]){
      if(row.RowType==='Row'){
        ...
        const amount=numVal(row.Cells?.[row.Cells.length-1]?.Value)
        if(name&&amount>0) rows.push({name,amount})
      }
    }
...
// Returns parsed totals, filtered by a mapping of included line names
export async function getXeroProfitAndLoss(accessToken,tenantId,months,mapping){
  const data=await fetchPL(accessToken,tenantId,months)
  return parsePL(data,months,mapping)
}

function parsePL(data,months,mapping){
  const report=data.Reports?.[0]
  if(!report) return null
  ...
  for(const sec of report.Rows||[]){
    ...
    for(const row of sec.Rows||[]){
      ...
      const name=row.Cells?.[0]?.Value||''
      const v=numVal(row.Cells)
      // If mapping provided, skip lines not included
